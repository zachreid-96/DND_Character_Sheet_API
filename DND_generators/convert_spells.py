class Spell:
    ...
    def adjust_spell(self):

        self.level = int(self.level)
        self.range = int(self.range)

        if self.reaction_condition == "":
            self.reaction_condition = [None]

        if self.shape_size != None:
            self.shape_size = int(self.shape_size)

        new_classes = []

        for _class in self.classes:
            new_classes.append(_class.split("_")[1].capitalize())
