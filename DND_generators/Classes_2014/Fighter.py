...
class Fighter2014:
    ...
    def __init__(self):
        ...
        fighter_json = r"./jsons/SRD_2014_Condensed/Fighter.json"
        self.loader = Loader()
        ...
        self.loader.load_fighter()

    def build_character(self, level=1, use_finesse=False, fighting_style=None):

        self.name = self.loader.class_fighter['class_name']
        self.hit_die = self.loader.class_fighter['hit_die']
        self.level = level

        if use_finesse:
            self.strength = self.loader.class_fighter['starting_abilities']['finesse_archer_based']['strength']
            self.dexterity = self.loader.class_fighter['starting_abilities']['finesse_archer_based']['dexterity']
            self.constitution = self.loader.class_fighter['starting_abilities']['finesse_archer_based']['constitution']
            self.intelligence = self.loader.class_fighter['starting_abilities']['finesse_archer_based']['intelligence']
            self.wisdom = self.loader.class_fighter['starting_abilities']['finesse_archer_based']['wisdom']
            self.charisma = self.loader.class_fighter['starting_abilities']['finesse_archer_based']['charisma']
        else:
            self.strength = self.loader.class_fighter['starting_abilities']['strength_based']['strength']
            self.dexterity = self.loader.class_fighter['starting_abilities']['strength_based']['dexterity']
            self.constitution = self.loader.class_fighter['starting_abilities']['strength_based']['constitution']
            self.intelligence = self.loader.class_fighter['starting_abilities']['strength_based']['intelligence']
            self.wisdom = self.loader.class_fighter['starting_abilities']['strength_based']['wisdom']
            self.charisma = self.loader.class_fighter['starting_abilities']['strength_based']['charisma']
        ...
        self.attacks = self.loader.class_fighter['attacks']
        self.action_surge = self.loader.class_fighter['action_surge']
        self.indomitable = self.loader.class_fighter['indomitable']
        self.crit_chance = self.loader.class_fighter['crit_chance']

        self.proficiency_bonus = self.loader.class_fighter['bonuses']['proficiency_bonus']
        self.ranged_bonus = self.loader.class_fighter['bonuses']['ranged_bonus']
        self.ac_bonus = self.loader.class_fighter['bonuses']['armor_class_bonus']
        self.onehanded_bonus = self.loader.class_fighter['bonuses']['one_handed_bonus']

        self.proficiencies = self.loader.class_fighter['proficiencies']
    ...
    def _get_starting_equipment(self):
        ...
        self.armor = {'Chain Mail': self.loader.item_armor['Heavy Armor']['Chain Mail']}
        self.armor_class = (self.loader.item_armor['Heavy Armor']['Chain Mail']['base_ac'])

        best_shield_ac = 0
        best_shield = None

        for shield in self.loader.item_armor['Shields']:
            if self.loader.item_armor['Shields'][shield]['min_strength'] > self.strength:
                continue
            if self.loader.item_armor['Shields'][shield]['armor_class_bonus'] > best_shield_ac:
                best_shield_ac = self.loader.item_armor['Shields'][shield]['armor_class_bonus']
                best_shield = {shield: self.loader.item_armor['Shields'][shield]}
        ...
        for category in self.loader.item_weapons:
            if any('martial' in elem.lower() for elem in category.split()):
                for weapon in self.loader.item_weapons[category]:
                    if "Two-Handed" in self.loader.item_weapons[category][weapon]['properties']:
                        continue
    ...
    def level_up(self, level: int):
        ...
        features = self.loader.class_fighter['level_features'][f'level_{level}']
        self.proficiency_bonus = features['proficiency_bonus']
        self.attacks = features['attacks']
        self.action_surge = features['action_surge']
        self.indomitable = features['indomitable']

        if features['asi_bonus'] > 0:
            if level == 4:
                self.strength += features['asi_bonus']
            if level == 6:
                self.strength += features['asi_bonus'] // 2
                self.constitution += features['asi_bonus'] // 2
            if level == 8:
        ...
        if level == 3:
            self.subclass = self.loader.class_fighter['subclasses']
            self.subclass_name = list(self.loader.class_fighter['subclasses'])[0]

        if self.subclass:
            if f"level_{level}" in self.loader.class_fighter['subclasses'][self.subclass_name]:
                level_data = self.loader.class_fighter['subclasses'][self.subclass_name][f"level_{level}"]
                self.crit_chance = level_data['crit_chance']
    ...
    def print_char(self):
        ...
        print(f"Attacks: {self.attacks} \tAction Surge: {self.action_surge} \tIndomitable: {self.indomitable}")
